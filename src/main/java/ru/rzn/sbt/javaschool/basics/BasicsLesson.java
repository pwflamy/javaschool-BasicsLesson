...
public class BasicsLesson {
    ...
    public Object newClass() {
        Object o = null;
        o = new LethalWeapon("red",100.0,10);
        return o;
    }
    ...
    public int stringExercise2(String input) {
        ...
        return input.length()*input.indexOf('R')*input.lastIndexOf('L')*zap*def*space;
    }
    ...
    public String stringExercise3(String input) {
        String s = input.trim();
        int countWord = s.length() - s.replaceAll(" ","").length() + 1;
        switch (countWord) {
            case 2: {
                if (s.contains("Hello")) {
                    return new StringBuilder(s).reverse().toString();
                }
                else if (s.contains("Goodbye")) {
                    return (s.substring(s.indexOf(" ")+1) + " " + s.substring(0,s.indexOf(" "))).toLowerCase();
                }
                break;
            }
            case 3: {
                String vrem = s.substring(0, s.indexOf(" ")) + s.substring(s.lastIndexOf(" "));
                if (s.contains("Hello")) {
                    return vrem;
                }
                else if (s.contains("Goodbye")) {
                    if (vrem.endsWith("!")) {
                        return vrem.substring(0,vrem.length()-1);
                    }
                    else {
                        return vrem += "!";
                    }
